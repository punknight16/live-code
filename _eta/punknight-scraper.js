...
function run () {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await page.addScriptTag({ content: `${generateId} ${crawlNode} ${addAttributes}`});
...
run().then(function(results){
    writeFile(results, function(err){
...
function writeFile(file_data, cb){
  fs.writeFile(__dirname+'/test.json', JSON.stringify(file_data), 'utf8', function(err){
    return cb(err)
  });
}
